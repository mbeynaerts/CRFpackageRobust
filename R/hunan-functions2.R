...
Meat <- function(coef.vector, X1, X2, datalist, Sl = NULL) {

  logtheta <- WoodTensor(X1, X2, coef.vector = coef.vector)

  if (!is.null(Sl)) penalty <- t(coef.vector) %*% Sl
  else penalty <- rep(0,length(coef.vector))

  score <- meat(riskset1 = datalist$riskset1,
                riskset2 = datalist$riskset2,
                logtheta1 = c(t(logtheta)),
                logtheta2 = c(logtheta),
                delta1 = datalist$delta1,
                ...
                idxN1 = datalist$idxN1 - 1,
                idxN2 = datalist$idxN2 - 1,
                penalty = penalty) # gradientC returns vector of derivatives of -loglik
...
EstimatePenal2 <- function(datalist, dim, degree = 3, lambda.init = c(1,1), start = rep(1,dim^2),
  ...
  if (robust) {
    U <- Meat(fit$beta, obj1$X, obj2$X, datalist, lambda.new[1]*S1 + lambda.new[2]*S2)
    robustvcov <- vcov %*% U %*% vcov
  } else robustvcov <- NULL

  return(list(
    beta = fit$beta,
    lambda = lambda.new,
    vcov = vcov,
    robustvcov = robustvcov,
    iterations = iter,
    ll = fit$ll,
